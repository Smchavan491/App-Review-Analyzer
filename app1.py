...
def predict():
    try:
        ...
        # Clean app_id in case full URL is given
        if input_text.startswith("http"):
            from urllib.parse import urlparse, parse_qs
            parsed_url = urlparse(input_text)
            query_params = parse_qs(parsed_url.query)
            input_text = query_params.get('id', [''])[0]
        ...
        return render_template('results.html', recommendation=recommendation, rating=avg_rating, user_image=pic2)
